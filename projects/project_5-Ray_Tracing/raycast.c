...
void shoot(float *origin, float *direction, float *color, int recLevel) {    
    ...
    if (objects[nearObj].reflect > 0) {
		...
		/*
    	if (objects[objNum].type == PLANE) {
        	setArray(newSurfNorm, objects[objNum].value.normal);              
    	}

    	else if (objects[objNum].type == SPHERE) {
        	v3_subtract(newSurfNorm, origin, objects[objNum].position);  
    	}
  		*/

		v3_subtract(newSurfNorm, newOrigin, objects[nearObj].position); // OBJECT POSITION NEEDED HERE
		...
		v3_reflect(newDirection, direction, newSurfNorm); // FOR SOME REASON THE NEW REFLECT VECTOR IS GETTING 0 VALUES
